...
class _WaitingJoinRoomState extends State<WaitingJoinRoom>
  ...
  Future<void> createAndJoinMeeting(displayName) async {
    int totalMinuteLive = ((widget.course.end!.millisecondsSinceEpoch - widget.course.start!.millisecondsSinceEpoch) / 60000).ceil();
    ...
    if(point! >= minPoint){
      ...
      showSnackBarMessage(
          message: 'กรุณาทำการตรวจสอบ Solve Point ของท่าน!\nจำนวน Solve Point ขั้นต่ำ : $minPoint',
          context: context);
    }
  }
  ...
  Future<void> updateActualTime() async {
    ...
    element.start?.compareTo(
        DateTime.fromMillisecondsSinceEpoch(widget.course.start!.millisecondsSinceEpoch)) ==
        0);
